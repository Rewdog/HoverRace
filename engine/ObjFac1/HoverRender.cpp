...
HoverRender::HoverRender(const Util::ObjectFromFactoryId &pId,
	...
	SUPER(pId)
{
	...
	mActor4 = resourceLib.GetActor(MR_NOVA_CRAFT);
	...
	BOOL pMotorOn, int pHoverId, unsigned int pModel)
{
	...
	if(pDest->ComputePositionMatrix(lMatrix, pPosition, pOrientation, 1000 /* TODO Object ray must be precomputed at compilation */ )) {
		...
		if(pModel == 1) {
			...
			// Nova: its own mesh, generated by res/meshes/gen-nova.ts and
			// compiled into ObjFac1.dat alongside the original four.
			ResActorFriend::Draw(mActor4, pDest, lMatrix, lSeq, mFrame, mCockpitBitmap2[pHoverId % 10]);
		} else {
